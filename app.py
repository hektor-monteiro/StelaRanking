...
# 2.2 Escolha dos Tipos de Produção do Eixo 3
...
st.markdown("**Categorias do Eixo 3 (Produção Ampliada):**")
target_production_types = st.multiselect("Selecione os itens para o Eixo 3:", options=todas_opcoes_eixo3, default=opcoes_padrao_eixo3, disabled=not eixo3_active)

# 2.3 Escolha dos Tipos de Orientação do Eixo 4
...
st.markdown("**Categorias do Eixo 4 (Orientações):**")
target_advising_types = st.multiselect("Selecione as modalidades para o Eixo 4:", options=todas_opcoes_eixo4, default=opcoes_padrao_eixo4, disabled=not eixo4_active)

# 2.4 Escolha do Método Matemático
st.markdown("**Método de Agregação:**")
metodo_calculo = st.radio("Cálculo da Nota Final (após normalização 0-1):", options=["Soma (Aditivo)", "Média (Compensatório)"], horizontal=True)
...
if file_docentes and file_prod and file_pessoas:
    if st.button("🚀 Processar Dados e Gerar Ranking", use_container_width=True):
        ...
        with st.spinner("A cruzar dados institucionais e processar Lattes..."):
            try:
                # 3.1 Leitura dos Ficheiros
                ...
                # Planilha de Pessoas (Áreas e Índices H)
                df_pessoas = load_file(file_pessoas, skip=3, separator=',')
                df_pessoas['Nome'] = df_pessoas['Nome'].str.strip()

            except Exception as e:
                st.error(f"Erro na leitura dos ficheiros: {e}")
                st.stop()

            # 3.2 Lógica de Métricas
            ...
            for prof in professors:
                ...
                # CÁLCULO CORRIGIDO: Restaurada a multiplicação pelo peso de cada estrato Qualis
            ...
            # 3.3 Join com Planilha de Pessoas (Áreas de Titulação e H-Index)
            df_final = pd.merge(df_res, df_pessoas[['Nome', 'Área da titulação máxima informada no CV-Lattes', 'Índice H (todos os anos)']], 
                                left_on='Docente', right_on='Nome', how='left')
            
            df_final.rename(columns={'Área da titulação máxima informada no CV-Lattes': 'Área de Titulação', 'Índice H (todos os anos)': 'Índice H'}, inplace=True)
            df_final['Área de Titulação'] = df_final['Área de Titulação'].fillna('Não Informada')
            df_final['Índice H'] = df_final['Índice H'].fillna(0)

            # 3.4 Normalização 0-1
            for col, abs_col in zip(['E1_N', 'E2_N', 'E3_N', 'E4_N'], ['E1_Abs', 'E2_Abs', 'E3_Abs', 'E4_Abs']):
            ...
            with col_res1:
                ...
                cols_view = ['Posição Ranking', 'Docente', col_final, 'Área de Titulação', 'Índice H']
                st.dataframe(df_final[cols_view].head(15), use_container_width=True, hide_index=True)
            
            with col_res2:
                st.subheader("💾 Exportação")
                st.download_button("📥 Descarregar Ranking Completo (.xlsx)", data=excel_buffer.getvalue(), file_name="ranking_pibic_areas_oficiais.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", use_container_width=True)
    ...
    st.warning("⚠️ Aguarde o upload das três planilhas para iniciar.")
